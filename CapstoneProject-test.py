...
print elapsed, 'time take to read the dataset'
...
elapsed = timeit.default_timer() - start_time
print elapsed, 'time taken for procressing' 
...
elapsed = timeit.default_timer() - start_time
print elapsed, 'time taken for tfidf'
...
elapsed = timeit.default_timer() - start_time
print elapsed, 'timr take fr label encoder'
...
elapsed = timeit.default_timer() - start_time
print elapsed, 'time takken for NMF'


X_train,X_test,y_train,y_test = train_test_split(boost_input,y,test_size=0.2, random_state=42)


model = XGBClassifier(silent=True,seed=42, scale_pos_weight=0.1 , nthread=-1)
param_grid = { 'colsample_bylevel':[0.5,0.6,0.7] }
...
print elapsed, 'time taken by gridsearchcv.fit'
...
elapsed = timeit.default_timer() - start_time
print elapsed, 'timr take to predict'
